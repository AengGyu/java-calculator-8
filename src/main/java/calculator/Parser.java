...
/**
 * parseSeparator : 기본 구분자를 제외한 커스텀 구분자를 파싱해서 리스트로 반환 parseOperands : 피연산자를 파싱해서 리스트로 반환
 */
public class Parser {

    /**
     * 문자열을 입력 받아서 커스텀 구분자가 있는 경우 커스텀 구분자와 기본 구분자를 포함하여 Set에 저장 후 반환 커스텀 구분자가 없는 경우 기본 구분자만 포함한 Set 반환
     */
    ...
    /**
     * // 와 \n 사이의 입력된 커스텀 구분자 문자열을 반환하는 메서드 메서드 이름 뭘로 하지 ??????
     */
    private static String getBeforeParsing(String input) {
    ...
    private static Set<Character> getCustomSeparators(String input) {
        ...
        for (int i = 0; i < customSeparators.length(); i++) {
            ...
            if (customSeparators.charAt(i) == '-') {
                throw new IllegalArgumentException("'-'는 구분자로 사용할 수 없습니다.");
            }
    ...
    /**
     * 커스텀 구분자가 있으면 떼어낸 후 피연산자를 하나씩 리스트에 저장 후 리턴 구분자에 포함되지 않는 문자가 피연산자 사이에 있는 경우 예외처리
     */
    ...
    /**
     * 계산식을 반환하는 메서드 커스텀 구분자가 없다면 input을 그대로 반환 커스텀 구분자가 있으면 커스텀 구분자가 입력된 부분을 제외한 계산식만 반환
     */
     ...
     */
    private static void checkOperator(char operand) {
        if (operand == '-') {
